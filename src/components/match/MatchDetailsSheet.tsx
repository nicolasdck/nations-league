...
export default function MatchDetailsSheet({
	match,
	teamsById,
	favoriteTeamId,
	onClose,
	...
	const [tab, setTab] = useState<DetailsTab>(
		match.status === 'NS' || match.status === 'PST' ? 'h2h' : 'timeline',
	);
	...
	return (
		...
		>
			...
			>
				<div className="relative px-4 pt-2 pb-2 flex flex-col gap-5">
					<div className="flex items-center">
						<button
							onClick={onClose}
							className="absolute right-3 text-slate-500 text-3xl leading-none"
							aria-label="Fermer"
						>
							×
						</button>
				...
				<div className="overflow-y-auto overscroll-contain px-4 pt-2 pb-[max(1.5rem,env(safe-area-inset-bottom))] min-h-48">
					...
					{details && tab === 'stats' && (
						<StatsSection details={details} sides={sides} />
					)}
					{details && tab === 'lineups' && (
						<LineupsSection details={details} sides={sides} />
					)}
					{details && tab === 'h2h' && (
						<HeadToHeadSection details={details} sides={sides} />
					)}
				</div>
			</div>
		</div>
